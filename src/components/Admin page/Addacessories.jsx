...
export default function Addacessories() {
  ...
  useEffect(() => {
    if (prams.slug) {
      axios.get(`http://localhost:8000/api/acessories/${prams.slug}`,{
        headers: {
  ...
  function submithandel(e) {
    ...
    let method = "post"
    let url = "http://localhost:8000/api/acessories"

    if (prams.slug) {
      method = "put"
      url = `http://localhost:8000/api/acessories/${prams.slug}`
    }
